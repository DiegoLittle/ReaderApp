    ...
    function replace_cskgs(str, link) {
      ...
      if (link.span.toUpperCase() == link.span) {
        ...
        if (link.link.name.toUpperCase() == link.span) {
          // console.log("ACRONYM")
          str = str.replace(link.span, `<a href=/wiki/${link.link.name} target="_blank" className="font-bold relative border-2 cursor-pointer border-red-400 bottom-1">${link.link.name}</a>`)
    ...
    }
    {
      ...
      links.forEach((link,index) => {
        ...
        if (link.link.type == "resource") {
          ...
          if (str.split(' ').includes(link.link.name)) {
          ...
          if (str.includes(link.span)) {
            ...
            if (link.link.name.split(' ').length == 1) {
              if (str.split(' ').includes(link.span)) {
                // let href = "https://paperswithcode.com/" + link.link.type + "/" + link.link.name.toLowerCase().replace(" ", "-")
