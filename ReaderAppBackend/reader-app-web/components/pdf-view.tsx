...
const PDFView = ({ url, setTitle, paper, arxiv_id=false }) => {
  ...
  async function onDocumentError(){
    console.log("error")
    let req = await fetch("/api/papers/queue",{
      method: 'POST',
      body:JSON.stringify({
        url: url,
        html_rendered: false,
  ...
  function replace_jsx(element,span,replace){
    let children = element.props.children
    console.log(children)
    // str = str.replace(link.span, `<a href=/wiki/${link.link.name} target="_blank" className="font-bold relative border-2 cursor-pointer border-blue-400 bottom-1">${link.link.name}</a>`)
    for (let index = 0; index < children.length; index++) {
      const jsx_item = children[index];
      if(typeof(jsx_item) == 'string'){
        if(jsx_item.includes(span)){
          var start_pos = jsx_item.indexOf(span)
          var end_post = start_pos+span.length

        }
      }
      console.log(jsx_item)
  ...
  const textRenderer = (render) => {
    ...
    // let elements = [<span>{str}</span>]
    // let base_element = <span>{str}<a>Hello World</a></span>
    
    // link.pages = [0,1,2,3]
    function str_text(str){
      let text = ""
      // console.log(str)
      try {
        str.forEach((item)=>{
    ...
    function replace_cskgs(str, link) {
      ...
      if (link.span.toUpperCase() == link.span) {
        ...
        if (link.link.name.toUpperCase() == link.span) {
          // console.log("ACRONYM")
          // str = str.replace(link.span, `<a href=/wiki/${link.link.name} target="_blank" className="font-bold relative border-2 cursor-pointer border-blue-400 bottom-1">{link.link.name}</a>`)
          // replace_jsx(base_element,link.span,link.link.name)
          // console.log(link.span)
          // console.log(str_text(str).split(" "))
          if(str_text(str).split(' ').includes(link.span)){
            // console.log("Link span : ", link.span)
            str = reactStringReplace(str, link.span, (match, i) => (
              <span className='group'>
                <a href={`/wiki/${link.link.name}`} target="_blank" className="font-bold relative border-2 cursor-pointer border-red-400 bottom-1">{link.link.name}</a>
                {/* <span className='invisible absolute text-left p-2 text-black z-10 w-40 h-40 bg-white group-hover:visible'>
                  {link.link.name}
                </span> */}
              </span>
            ));
        ...
        found_entities.push(link.link.name)
        // str = str.replace(link.span, `<a href=/wiki/${link.link.name} target="_blank" className="relative border-2 cursor-pointer border-blue-400 bottom-1">${link.link.name}</a>`)
        // replace_jsx(base_element,link.span,link.link.name)
        // str = 
      }
      return {
    ...
    }
    {
      ...
      links.forEach((link,index) => {
        ...
        if (link.link.type == "resource") {
          ...
          // if entity type is method,task, etc
          
          // Link is from the papers with code
          // Splits string token by space
          // Tests if any of the tokens are a match to the linked label
          
          try{
            if (str_text(str).split(' ').includes(link.link.name)) {
              entities_found.push(link.link.name)
              // console.log(link.link.name)
              // let href = "https://paperswithcode.com/" + link.link.type + "/" + link.link.name.toLowerCase().replace(" ", "-")
              // str = str.replace(link.span, `<a href=${link.link.url} target="_blank" className="relative border-2 cursor-pointer border-blue-400 bottom-1">${link.link.name}</a>`)
              // replace_jsx(base_element,link.span,link.link.name)
              str =reactStringReplace(str, link.span, (match, i) => (
                <a href={`/wiki/${link.link.name}`} target="_blank" className="font-bold relative border-2 cursor-pointer border-blue-400 bottom-1">{link.link.name}</a>
              ));
            }
          }
          catch(e){
            console.log(e)
            // console.log(str_text(str))
            // console.log(str)
          }

          if (str_text(str).includes(link.span)) {
            entities_found.push(link.link.name)            
            // console.log(str)
            includes_kw = true;
            // console.log(link.link.type)
            // console.log(link.link.name.toLowerCase().replace(" ","-"))

            if (link.link.name.split(' ').length == 1) {
              if (str_text(str).split(' ').includes(link.span)) {
                // let href = "https://paperswithcode.com/" + link.link.type + "/" + link.link.name.toLowerCase().replace(" ", "-")
                // str = str.replace(link.span, `<a href=${link.link.url} target="_blank" className="relative border-2 cursor-pointer border-blue-400 bottom-1">${link.span}</a>`)
                // replace_jsx(base_element,link.span,link.link.name)
                str =reactStringReplace(str, link.span, (match, i) => (
                  <a href={`${link.link.url}`} target="_blank" className="font-bold relative border-2 cursor-pointer border-blue-400 bottom-1">{link.span}</a>
                ));
              ...
              // str = str.replace(link.span,`<a href="/wiki/${link.link.name}?type=${link.link.type}" target="_blank" className="relative border-2 cursor-pointer border-blue-400 bottom-1">${link.span}</a>`)
              str =reactStringReplace(str, link.span, (match, i) => (
                <a href={`/wiki/${link.link.name}`} target="_blank" className="font-bold relative border-2 cursor-pointer border-blue-400 bottom-1">{link.span}</a>
              ));
            }
...
const DocumentView = memo(({ url, onDocumentLoadSuccess,onDocumentError, totalPages, setSelectedText, textRenderer, removeTextLayerOffset }) => {
