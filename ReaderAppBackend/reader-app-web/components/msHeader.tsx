      ...
      <div className="flex justify-between items-center px-4 py-6 sm:px-6 md:justify-start md:space-x-10">
        <div>
          <a href="/" className="flex">
            <span className="font-semibold text-xl ">Synesthesia Reader</span>
            {/* <img
          ...
          {navigation.map((link) =>
          <Link href={link.href}>
            <a  className="md:hidden text-base font-medium text-black hover:text-gray-900 mx-4">
              {link.name}
            </a>
            </Link>
          )}
...
{/* 
        ...
        <div className="hidden md:flex-1 md:flex md:items-center md:justify-between">
          <Popover.Group as="nav" className="flex space-x-10">
            ...
            {navigation.map((link) =>
            <Link href={link.href} >
              <a  className="text-base font-medium text-black hover:text-gray-800">
                {link.name}
              </a>
              </Link>
            )}
              ...
              <div>
                <Menu.Button className=" flex text-sm rounded-full text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white">
                  ...
                  {/* <img
                              ...
                              alt=""
                            /> */}
                  {session.user.image ? <Image width={40} height={40} className='rounded-full cursor-pointer' src={session.user.image}></Image> : <UserIcon className='w-8 h-8 text-blue-700'></UserIcon>}
                </Menu.Button>
              </div>
              <Transition
                as={Fragment}
        ...
        <Popover.Panel focus className="absolute z-20 top-0 inset-x-0 p-2 transition transform origin-top-right md:hidden">
          <div className="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 bg-white divide-y-2 divide-gray-50">
            ...
            <div className="py-6 px-5">
              ...
              {session ? <>
              {session.user.image?
                <Image width={20} height={20} src={session.user.image}></Image>:<div>No Image</div>
              }
              </> : <>
                <div className="">
