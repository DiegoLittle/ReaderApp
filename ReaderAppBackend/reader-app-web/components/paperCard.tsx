...
export default function PaperCard({ paper, setPapers,papers }) {
  const ref = useRef()
  ...
  async function fetch_infinit_scroll(){
    let papers_req = await fetch('/api/papers?skip='+papers.length)
  ...
  return (
    <Link href={`/read/${arxiv_id}`}>
      <div ref={ref} className="flex bg-white flex-col md:mx-6 mt-10 p-6 rounded-lg shadow-lg cursor-pointer overflow-hidden hover:bg-blue-50" >
