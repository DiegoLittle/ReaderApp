...
const Home: NextPage = () => {
    ...
    async function search(intent){
      ...
      try {
        ...
        // let res = await fetch(process.env.NEXT_PUBLIC_API_URL+'/api/paper_search?q='+query)
        let res = await fetch("http://localhost:8000/api/search?intent="+ intent +"&q="+query)
        let data = await res.json()
        console.log(data)
    ...
    <div className="flex min-h-screen flex-col items-center justify-center">
      ...
      <main className="flex w-full flex-1 flex-col ">
          ...
          {loadingResults?
              ...
              <>
                        {intent == "papers" ? results.map((paper)=>
            <PaperCard paper={paper}></PaperCard>
          ):<div className=' flex-col'>
            {results.map((page)=>
            <div className='mx-auto w-3/4 bg-white flex-col md:mx-6 mt-10 p-3 rounded-lg shadow-lg cursor-pointer overflow-hidden hover:bg-blue-50'>
              <div className='text-lg font-semibold'>{page.title}</div>
              {page.type && 
              <div className='inline-flex mr-2 bg-blue-300 rounded-lg p-1 text-sm cursor-pointer'>
              <Link href={"/wiki/" + page.type}>
                <span >{page.type[0].toUpperCase()+page.type.substring(1)}</span>
              </Link>
            </div>
              }
