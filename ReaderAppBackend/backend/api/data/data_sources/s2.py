...
def fetch_papers_no_s2(worker=0):
    connection = engine.connect()
    if worker == 0:
        result = connection.execute("SELECT arxiv_id,title,abstract,date,s2_paper FROM papers WHERE arxiv_id IS NOT NULL AND s2_paper IS NULL AND num_citations IS NULL ORDER BY date DESC LIMIT 100;")
    else:
        print("Worker {}".format(worker))
        result = connection.execute("SELECT arxiv_id,title,abstract,date,s2_paper FROM papers WHERE arxiv_id IS NOT NULL AND s2_paper IS NULL AND num_citations IS NULL ORDER BY date DESC LIMIT 100 OFFSET " + str(worker*100) + ";")
    return result.fetchall()
...
def get_s2_data(worker):
    if api_key is None:
        ...
        while True:
            ...
            papers = fetch_papers_no_s2(worker)
            start = time.time()
            for paper in papers:
                if num_requests >= 100:
                    end = time.time()
